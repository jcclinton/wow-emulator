...
-export([get_guid/0]).
-export([add_to_map/2, remove_from_map/1, get_map_players/0]).
...
add_to_map(Name, Msg) ->
	gen_server:cast(?MODULE, {add_to_map, Name, Msg}).
...
handle_cast({add_to_map, Name, Msg}, State = #state{players=Players}) ->
